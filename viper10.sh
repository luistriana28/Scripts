...
for d in $(cat /home/.backups/c10.txt)
do
    ...
    if [ ! -d ${path} ]; then
     ...
     do
         if [ "$i" != "template0" ] && [ "$i" != "template1" ] && [ "$i" != "postgres" ] && [ "$i" != "?" ] && [ "$i" != " " ] ; then
             ...
             docker cp ${d}:/home/odoo-10.0/.local/share/Odoo/filestore/filestore/$i/ $path
         fi
     done
     cd /home/.backups
     tar -cvf - ${d} | gzip > /home/.backups/${d}_$date.tar.gz
     rm -rf ${d}
     mkdir -p backup10_${date}
     mv *.tar.gz /home/.backups/backup10_${date}
